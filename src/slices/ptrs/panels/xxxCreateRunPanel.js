...
export default function CreateRunPanelxxx() {
    ...
    try {
      ...
      if (requiresCsv && file) {
        ...
        if (isBoth) {
          ...
          goTo(`xero?ptrsId=${encodeURIComponent(newRunId)}`, {
            includeId: false,
          });
          return;
        }
      ...
      goTo(`data?${qs.toString()}`, { replace: true, includeId: false });
  ...
  const goNext = () => {
    ...
    qs.set("runId", runId);
    if (profileId) qs.set("profileId", profileId);
    goTo(`tables?${qs.toString()}`, { includeId: false });
  };
