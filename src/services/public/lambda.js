...
function buildPlainTextBodyFromPayload(parsed) {
  return [
    `--= Start of detail block`,
    `* Subject: ${parsed.subject || "Not provided"}`,
    `* Message: ${parsed.message || "Not provided"}`,
    `* Name: ${parsed.name || "Not provided"}`,
    `* Email: ${parsed.email || "Not provided"}`,
    `* Date: ${parsed.date || "Not provided"}`,
    `* Time: ${parsed.time || "Not provided"}`,
    `* Company: ${parsed.company || "Not provided"}`,
    `* From: ${parsed.from || parsed.email || "Not provided"}`,
    `* CC: ${parsed.cc || "Not provided"}`,
    `* BCC: ${parsed.bcc || "Not provided"}`,
    `--= End of detail block`,
  ].join("\n");
}
...
export const handler = async (event) => {
  ...
  try {
    ...
    if (event.body && !event.Records) {
      ...
      console.log("Parsed frontend payload:", data);

      // These links/signature should match those in the SESInBoundProcessor
      const homeLink = "https://monochrome-compliance.com";
      const privacyPolicyLink =
        "https://monochrome-compliance.com/policy-documents/privacy-policy";
      const signUpLink = "https://monochrome-compliance.com/signup";
      ...
      const communityLink = "https://monochrome-compliance.com/community";
      const adminDashyLink =
        "https://monochrome-compliance.com/admin-dashboard";
      const stdDashyLink = "https://monochrome-compliance.com/dashboard";
      ...
      // Build the contactEmail object as the single source of truth for all fields
      const contactEmail = {
        to: data.to || "contact@monochrome-compliance.com",
        subject:
          data.subject ||
          (data.topic ? data.topic + " " + new Date().toISOString() : ""),
      ...
      const extractedSubject = (contactEmail.subject || "").trim();
      const service_request_number = extractTrimmedNumbers(extractedSubject);
      const clientName = contactEmail.name || "there";
      const bookingDate = contactEmail.date || "";
      const bookingDateFormatted =
        bookingDate && /^\d{4}-\d{2}-\d{2}$/.test(bookingDate)
          ...
          : bookingDate;
      const bookingTime = contactEmail.time || "";
      const clientCompany = contactEmail.company || "";
      const fromSource = (contactEmail.from || "").toLowerCase();
      const toAddress =
        contactEmail.email ||
        contactEmail.from ||
        "fallback@monochrome-compliance.com";
      // Use contactEmail for the structured block
        ...
        <div style="margin-top:10px">
          ...
          <div style="margin-top:10px">
            <div style="margin-left: 25px; font-family: Arial, sans-serif; color: #333;">
              ...
              ${formatStructuredBlockForHtml(contactEmail, extractedSubject)}
            </div>
          </div>
        </div>
      `;
      ...
      // Replicate the SESInBoundProcessor auto-response logic for the frontend POST
      if (fromSource === "contact@monochrome-compliance.com") {
        ...
        htmlBody = `
          <html><body>
            <p>Hi ${clientName},</p>
            <p>Thanks for reaching out to Monochrome Compliance. We'll get back to you shortly.</p>
            <p>Warm regards,</p>
            ${emailSignature}
          </body></html>
        `;
      ...
      const BccAddress =
        "darryll.robinson@monochrome-compliance.com; mitch.pentz@monochrome-compliance.com";

      await ses.send(
        new SendEmailCommand({
          Destination: {
            ToAddresses: [toAddress],
            BccAddresses: BccAddress.split(";").map((addr) => addr.trim()),
          },
          Message: {
      ...
      console.log("SES send successful from frontend payload.");
      return {
        statusCode: 200,
        headers: {
        ...
        body: JSON.stringify({
          status: 200,
          message: "Frontend email processed successfully",
        }),
      };
    }
    // All SES inbound/S3/Records logic removed.
    // If event.Records is present, do nothing (or optionally return 200).
    return {
      statusCode: 200,
      headers: {
        ...(corsOrigin && { "Access-Control-Allow-Origin": corsOrigin }),
      ...
      body: JSON.stringify("No processing for SES inbound in this handler."),
    };
  } catch (error) {
    console.error(
