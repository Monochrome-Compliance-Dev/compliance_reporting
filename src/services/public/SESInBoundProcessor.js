...
export const handler = async (event) => {
  try {
    ...
    const pdfRecord = event.Records[0];
    const pdfKey = decodeURIComponent(
      pdfRecord.s3.object.key.replace(/\+/g, " ")
    );
    const pdfBucket = pdfRecord.s3.bucket.name;

    const getObjectParams = { Bucket: pdfBucket, Key: pdfKey };
    const { Body } = await s3.send(new GetObjectCommand(getObjectParams));
    const pdfRawEmail = await streamToString(Body);

    const boundaryMatch = pdfRawEmail.match(/boundary="([^"]+)"/i);
    if (!boundaryMatch) throw new Error("No MIME boundary found");

    const boundary = boundaryMatch[1];
    const pdfParts = pdfRawEmail.split(`--${boundary}`);
    ...
    for (const part of pdfParts) {
      if (part.includes("Content-Disposition: attachment")) {
        const pdfNameMatch = part.match(/name="([^"]+)"/i);
        const smtpId = extractSmtpId(pdfRawEmail);
        const pdfFilename =
          smtpId ?? pdfNameMatch?.[1] ?? `attachment-${Date.now()}`;

        const contentMatch = part.match(/\r?\n\r?\n([\s\S]*)/);
        const rawBase64 = contentMatch ? contentMatch[1] : null;

        if (rawBase64) {
          const cleanedBase64 = rawBase64
            .replace(/\r?\n/g, "")
            .replace(/[^A-Za-z0-9+/=]/g, "");
          ...
          try {
            ...
            if (buffer.length < 1000) {
              console.warn(
                `Decoded PDF buffer is unusually small (${buffer.length} bytes). Skipping upload.`
              );
              continue;
            }

            const attachmentS3Key = `attachments/${pdfFilename}.pdf`;
            await s3.send(
            ...
            console.log(
              `Attachment ${pdfFilename}.pdf reuploaded successfully (${buffer.length} bytes).`
            );
            attachmentFilename = attachmentS3Key;

            // Fetch it back as base64 (safe after PutObject completes)
            attachmentBase64 = buffer.toString("base64");
          } catch (err) {
            console.error(
              "Failed to decode or reupload PDF attachment:",
              err.message
            );
          }
        } else {
          console.warn("No base64 content found in attachment part.");
        }
      }
    }
    ...
    if (isAutoReply(parsedHeaders)) {
      ...
      return { statusCode: 200, body: JSON.stringify("Skipped auto-reply.") };
    }
